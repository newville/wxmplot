...
class ImageFrame(BaseFrame):
    ...
    def BuildFrame(self, size=(550,450), **kwds):
        ...
        interp_choice =  wx.Choice(lpanel, -1, choices=Interp_List, size=(40,-1))
        ...
        cmap_choice =  wx.Choice(lpanel, -1, choices=ColorMap_List, size=(40,-1))
        ...
        cmap_toggle = wx.CheckBox(lpanel,-1, 'Reverse Table', (-1,-1),(-1,-1))
        ...
        self.cmap_fig   = Figure( (0.350, 1.25), dpi=100)
        self.cmap_axes  = self.cmap_fig.add_axes([0,0,1,1])
        self.cmap_axes.set_axis_off()
        
        self.cmap_canvas = FigureCanvasWxAgg(lpanel, -1, self.cmap_fig)

        bgcol =   rgb2hex(lpanel.GetBackgroundColour()[:3])

        self.cmap_fig.set_facecolor('#FF0033') # bgcol)
        ...
        mainsizer.Add(lpanel, 0,wx.LEFT|wx.ALIGN_LEFT|wx.TOP|wx.ALIGN_TOP|wx.EXPAND)
        if self.img_panel is not None:
            self.img_panel.BuildPanel()
            self.img_panel.messenger = self.write_message
            self.img_panel.fig.set_facecolor(bgcol)
    ...
    def UpdateImages(self):
        ...
        self.cmap_data[hi:,:] = 1
        self.cmap_data[:lo,:] = 0
        self.cmap_data[lo:hi] = numpy.outer(numpy.linspace(0,1,hi-lo),
                                            numpy.ones(cmax/8))
